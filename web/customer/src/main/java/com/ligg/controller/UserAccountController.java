...
public class UserAccountController {
    ...
    public Result<String> login(@RequestBody Map<String, String> request) {
        ...
        if (byAccount.getUserStatus() == 0) {
            return Result.error(BusinessStates.BAD_REQUEST, "该账号已被注销");
        }
    ...
    public Result<String> register(@RequestBody Map<String, String> user) {
        ...
        if (account.length() < 6 || password.length() < 6) {
            return Result.error(BusinessStates.BAD_REQUEST, "账号或密码长度不小于6位");
        }
        if (account.length() > 20 || password.length() > 20) {
            return Result.error(BusinessStates.BAD_REQUEST, "账号或密码长度不大于20位");
        }

        // 检查账号是否已存在
        if (userService.findByUser(account) != null) {
            return Result.error(BusinessStates.BAD_REQUEST, "账号已存在");
        }
        ...
        if (invitationCode != null && !invitationCode.isEmpty()) {
            ...
            if (inviterAccount == null) {
                return Result.error(BusinessStates.BAD_REQUEST, "邀请码无效");
            }
